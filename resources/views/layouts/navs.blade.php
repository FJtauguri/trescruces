...
<nav id="navbar" class="navbar fixed-top navbar-dark navbar-expand-lg justify-content-center align-items-center">
    <div class="container">
        <a class="navbar-brand" id="cstm-nav-brand" style="font-size:14px; border-radius:50%;" href="{{ url('/') }}">
            <img height="60"style="border-radius:50%;" src="{{ asset('assets/head_logo/' . ($gen->logo ?? 'default-logo.png')) }}" alt="Logo">
            {{ $gen->title }}
        </a>
        ...
        <div class="collapse navbar-collapse" id="navdrop">
            <ul class="navbar-nav mx-auto">
                ...
                <li class="nav-item mx-1">
                    <a class="nav-link" id="programs" href="{{ url('programs') }}">PROGRAM</a>
                </li>
                ...
                @foreach($navs as $data)
                    ...
                    @endphp
                    <li class="nav-item mx-1">
                        <a class="nav-link " href="{{ url($page_name) }}" style="text-transform:uppercase;">{{ $nav }}</a>
                    </li>
                @endforeach
