                ...
                @foreach ($messages as $message)
                    <div class="d-flex mb-2 {{ $message->sender_id === Auth::id() ? 'justify-content-end' : '' }}">
                        <div class="chat-bubble {{ $message->sender_id === Auth::id() ? 'bg-primary text-white' : 'bg-light' }}">
                            <h5>
                            {!! nl2br(preg_replace('/https?:\/\/[^\s<]+/', '<a style="color:red;" href="$0" target="_blank">$0</a>',$message->message)) !!}
                            </h5>
                           
                     <small  style="font-size:12px;"><em>{{ $message->created_at->diffForHumans() }}</em></small>
