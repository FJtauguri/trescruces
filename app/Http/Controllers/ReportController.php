    ...
    public function sendReport(Request $request)
{
    ...
    if ($request->hasFile('report_photo')) {
        ...
        // Define the path for the report photo
        $photoPath = 'report_photo/' . $photoName;
        
        // Store the file using the Storage facade
        Storage::disk('public_html')->putFileAs('report_photo', $photo, $photoName);
    }
    ...
    try {
        ...
        return redirect()->route('file-report.index')
            ->with('alert', ['type' => 'success', 'message' => 'Report submitted successfully! Please wait for a reply on your email']);
    } catch (\Exception $e) {
