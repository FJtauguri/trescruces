   ...
   public function exportSpecificRecordToExcel($recordId)
{
    ...
    // Check if `table_data` has valid data (not empty and contains non-null values)
    $tableData = json_decode($record->table_data, true);
    $hasValidData = !empty($tableData) && array_filter($tableData, fn($item) => array_filter($item, fn($value) => !is_null($value)));

    if ($hasValidData) {
        // Set headers for `table_data`
        $sheet->setCellValue('A4', 'ID');
        $sheet->setCellValue('B4', 'Surname');
        $sheet->setCellValue('C4', 'First Name');
        $sheet->setCellValue('D4', 'Middle Name');
        $sheet->setCellValue('E4', 'Birthday');
        $sheet->setCellValue('F4', 'Birthplace');
        $sheet->setCellValue('G4', 'Relationship');
        $sheet->setCellValue('H4', 'Occupation/Status');
        $sheet->setCellValue('I4', 'Civil Status');
        $sheet->setCellValue('J4', 'DO1D');
        $sheet->setCellValue('K4', 'DO2D');
        $sheet->setCellValue('L4', 'Vaccine');

        // Add table_data entries to the sheet
        $row = 5;
        foreach ($tableData as $data) {
            ...
            $sheet->setCellValue('E' . $row, $data['birthday'] ?? '');
            $sheet->setCellValue('F' . $row, $data['birthplace'] ?? '');
            $sheet->setCellValue('G' . $row, $data['rspnp'] ?? '');
            $sheet->setCellValue('H' . $row, $data['tglosy'] ?? '');
            $sheet->setCellValue('I' . $row, $data['pssp'] ?? '');
            $sheet->setCellValue('J' . $row, $data['do1d'] ?? '');
            $sheet->setCellValue('K' . $row, $data['do2d'] ?? '');
            $sheet->setCellValue('L' . $row, $data['vcne'] ?? '');
            $row++;
        }
    }
