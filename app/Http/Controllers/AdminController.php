...
class AdminController extends Controller
{
    public function showDashboard()
    {
        ...
        return view('admin.dashboard', compact('monthly_requests', 'male_count', 'female_count', 'uncat_count', 'na_count', 'resCount'));
    }
    ...
    public function viewResidents()
    {
        ...
        $users = User::where('roles', 'resident')
            ->where('verified', true)  // Filter by verified status
            ->get()
            ->map(function ($user) {
                return [
                    'Firstname' => $user->fname,
        ...
        // Log the activity for admin viewing the residents page
        ActivityLog::create([
            'user_id' => auth()->user()->id,
            'activity' => 'Visited View Residents page.'
        ]);

        // Return the view with the verified residents list
    ...
    public function approveUser(int $id)
    {
        ...
        // Event
        event(new Monitoring(" The request of {$user} has been approved!"));
